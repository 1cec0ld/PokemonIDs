    ...
    private void giveBadge(Player sender, Player target, String cityname) {
        ...
        if(value>0){
            ...
            if(!controller.getPlugin().getPlayerStorageManager().hasBadge(target.getUniqueId().toString(), regionname, value)){
                ...
                controller.getPlugin().getServer().dispatchCommand(controller.getPlugin().getServer().getConsoleSender(), "advancement grant " + target.getName() + " only pokemonserver:badges/"+regionname+"/"+cityname);
            } else {

...
public class DiveInteractListener implements Listener {
    ...
    public void onInteract(PlayerInteractEvent event){
        ...
        if(controller.permissionToBreak(event.getPlayer())){
            PokemonServer.msgActionBar(event.getPlayer(),"You used DIVE!",ChatColor.BLUE);
            diveBlockBreak(hitblock,id);
        } else {
            PokemonServer.msgActionBar(event.getPlayer(),"The water looks deep here.", ChatColor.RED);
        }
    }

    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if(args.length == 0){
            ...
            if(sender instanceof Player){
                ...
                if(player.hasPermission("pokemonserver.teleports.spawn.self")){
                    spawn(player);
                }
            } else {
                sender.sendMessage("Not applicable from console! Choose a target!");
            }
        } else if(args.length == 1 && (!(sender instanceof Player) || ((Player)sender).hasPermission("pokemonserver.teleports.spawn.others"))){
